...
import kotlinx.android.synthetic.main.fragment_main_workoutline.*
...
class MainWorkOutlineFragment : BaseFragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_main_workoutline, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        rightButton.setOnClickListener {
            val intent = Intent(context, CreateWorkeOutlineActivity::class.java)
            intent.putExtra("EMAIL", "")
            startActivityForResult(intent,1000)
            activity!!.overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left)
        }

        tvTitle.text = "Hạng Mục Thi Công"
    }
    ...
    private fun getData(page:Int){
        showProgessDialog()
        RestClient().getInstance().getRestService().getWorkOutlines(page,"").enqueue(object :
